...
[Imagestrain, IdentityTrain] = loadTestImagesInDirectory ( 'training-set/23x28/');
...
for i=1:20
    Eigenface=reshape(Space(i,:),[28,23]);
    subplot (4, 5, i);
    imshow(Eigenface,[-0.1 0.1]);
    title('Eigenface');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 
...
n=0;
for i=1:70
    if IdentityTrain(1,Indices(i,1))==Identity(1,i)
       n=n+1; 
    end
end
rate=n/70;    
...
for k=1:30
    n=0;
    for p=1:70
        tab=tabulate(IdentityTrain(1,Indices(p,1:k)));
        [val,index] =sort(tab(:,2),'descend');
        label=index(1,1);
        if label==Identity(1,p)
           n=n+1;
        end
    end
    KNNrate(1,k)=n/70;
...
figure;
plot(KNNrate(1,:));
title('Recognition rate against the number of K used');
